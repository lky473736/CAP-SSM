...
class DeLighTBlock(nn.Module):
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1, reduction_factor=4):
        super().__init__()
        self.local_proj = nn.Linear(d_model, d_model // reduction_factor)
        self.attn = MultiHeadAttention(d_model // reduction_factor, num_heads // 2, dropout)
        self.expand_proj = nn.Linear(d_model // reduction_factor, d_model)
        self.ff = nn.Sequential(nn.Linear(d_model, d_ff // reduction_factor), nn.ReLU(), nn.Dropout(dropout), nn.Linear(d_ff // reduction_factor, d_model))
        self.norm1, self.norm2 = nn.LayerNorm(d_model), nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        local_x = self.local_proj(x)
        attn_out = self.expand_proj(self.attn(local_x, local_x, local_x))
...
class DeLighTTransformer(nn.Module):
    def __init__(self, input_channels, num_classes, d_model, num_heads, num_layers, d_ff, dropout):
        ...
        self.layers = nn.ModuleList([DeLighTBlock(d_model, num_heads, d_ff, dropout) for _ in range(num_layers)])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build a DeLighT Transformer for HAR.")
    ...
    print("\n" + "="*50 + "\nBuilding DeLighTTransformer with Configuration:\n" + "="*50)
    for k, v in vars(args).items(): print(f"{k:<15}: {v}")
    print("="*50 + "\n")

    model = DeLighTTransformer(**vars(args))
