...
class PyramidAttention(nn.Module):
    def __init__(self, d_model, num_heads, dropout=0.1):
        super().__init__()
        self.coarse_attn = MultiHeadAttention(d_model, num_heads, dropout)
        self.fine_attn = MultiHeadAttention(d_model, num_heads, dropout)
        self.fusion = nn.Linear(d_model * 2, d_model)

    def forward(self, x):
        sl = x.shape[1]
        coarse_x = x[:, ::2, :] if sl > 1 else x
        coarse_out = self.coarse_attn(coarse_x, coarse_x, coarse_x)
        coarse_out = coarse_out.repeat_interleave(2, dim=1)[:, :sl, :] if sl > 1 else coarse_out
        fine_out = self.fine_attn(x, x, x)
        return self.fusion(torch.cat([coarse_out, fine_out], dim=-1))

class PyraformerBlock(nn.Module):
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1):
        super().__init__()
        self.pyramid_attn = PyramidAttention(d_model, num_heads, dropout)
        self.ff = nn.Sequential(nn.Linear(d_model, d_ff), nn.ReLU(), nn.Dropout(dropout), nn.Linear(d_ff, d_model))
        self.norm1, self.norm2 = nn.LayerNorm(d_model), nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        x = self.norm1(x + self.dropout(self.pyramid_attn(x)))
...
class Pyraformer(nn.Module):
    def __init__(self, input_channels, num_classes, d_model, num_heads, num_layers, d_ff, dropout):
        ...
        self.layers = nn.ModuleList([PyraformerBlock(d_model, num_heads, d_ff, dropout) for _ in range(num_layers)])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build a Pyraformer for HAR.")
    ...
    print("\n" + "="*50 + "\nBuilding Pyraformer with Configuration:\n" + "="*50)
